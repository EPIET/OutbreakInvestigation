...
*Format of combined onset_datetime variable:*
...
However, a new data cleaning problem has come to light; it seems there were two visitors from the future (2020)!  They are not cases (remember that some respondents had non-gastrointestinal symptoms and would still have recorded an onset date but won't meet the case definition).

We can review their records to see what happened:

```{r view_future_visitors, eval=FALSE}
...
Surprisingly, `dayonset` for these two cases is actually `2006-11-11`; but they are missing a `starthour`.  The error was introduced when we created the `onset_datetime` variable, due to the missing values for `starthour`. 

Hoever, we can also see that the two respondents were correctly categorised as non-cases; although they ate at the meal, they did not develop any symptoms.  We won't need to use onset dates for non-cases in our analysis, so we can ignore this error for now.  None the less, this highlights the importance of thinking about how best to deal with missing values `NA` when creating new variables.


**Questions to consider:**

  1. Why do you think this error occured (hint: we used the function `ymd_h()`)?
  
  2. How would you modify the code used to create `onset_datetime` to prevent this error from occurring?

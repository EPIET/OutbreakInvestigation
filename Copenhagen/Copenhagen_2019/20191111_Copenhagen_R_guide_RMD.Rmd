...
`str()` provides an overview of the structure of your data (number of observations and variable types):
...
`summary()` provides information on variable classes, as well as interquartile ranges for numeric variables:

```{r, echo = TRUE, warning=FALSE, output.lines = 14}
# Summarise data:
summary(cph)
```

`Hmisc::describe()` provides:
...
In the study, a **case** was defined as:
 ...
 - presenting with diarrhoea or vomiting
...
Using the "table" and "summary" commands, you can get the equivalent of "tabulate" and "summarize" in STATA. In the example below, we look at age in the `cph` dataset.  
...
In addition, the `ifelse(...)` function is very helpful; this function takes a conditional statement as its first argument, followed by the value to assign if that statement is true, and the value to assign if that statement is false.
...
- `NULL` is the symbol for an empty slot (e.g. the default for optional funciton arguments is NULL)
...
```{r}
# Create a new (empty) variable called case:
cph$case <- NA
...
# Assign 0 to those who had onset before 11th nov 18:00 or after 13th nov 18:00
cph$case[cph$case == 1 & 
            ((cph$dayonset == "11nov2016" & cph$starthour < 18) |
               (cph$dayonset == "13nov2016" & cph$starthour > 18) | 
                 cph$dayonset > "13nov2016")] <- 0

# Assign NA (missing value) to those who haven't been exposed to any food item

...
You can use this section to install and load the packages that you will need for this session.  Some brief details on why each package was selected are provided below:
   ...
   + `data.table` - fast data import, cleaning and data wrangling
...
To create a static map of cases, we will first import data to create a background map of the UK using the `osmdata` package.  You can define the boundary box for the map you want to import simply by typing the country name (or region and country name if you want to focus on one region).  
...
```{r interactive leaflet map, message=FALSE, warning=FALSE}
